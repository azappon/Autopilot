...
trained_model_path = "C:\\Users\\39347\\Desktop\\Self-Driving-Car-in-Video-Games-master\\PreTrained_Model"
...
def run_ted1104(
    ...
    """
    Generate dataset exampled from a hASuman playing a videogame
    HOWTO:
    ...
    while True:
        ...
        if "J" not in keys:
            ...
            X = torch.stack(
                (
                    transform(img_seq[0] / 255.0).float(),
                    transform(img_seq[1] / 255.0).float(),
                    transform(img_seq[2] / 255.0).float(),
                    transform(img_seq[3] / 255.0).float(),
                    transform(img_seq[4] / 255.0).float(),
                ),
                dim=0,
            ).to(device)
